...
状態を**ヘテロジーニアスグラフ** $\mathcal{G} = (\mathcal{V}, \mathcal{E})$ として表現する．ノード特徴量はペイロードの有無によって次元が変わる：

**ペイロードなし（`--no_payload`）：ノード特徴量 3 次元**
...
**ペイロードあり（デフォルト）：エージェントノードのみ 7 次元**

| ノード種別 | 特徴量 | 数 |
|---|---|---|
| Agent（スワーム） | $[1, 0, 0,\; \gamma_x,\; \gamma_y,\; \dot{\gamma}_x,\; \dot{\gamma}_y]$ | $n$ |
| Goal（目標位置） | $[0, 1, 0,\; 0, 0, 0, 0]$ | $n$ |
| Obstacle（LiDARヒット点） | $[0, 0, 1,\; 0, 0, 0, 0]$ | 可変 |
...
これを物理スケール（**加速度オフセット単位**）に変換する：

$$\pi_\text{scaled} = \pi_\phi \odot [a_\text{max}^\text{GNN},\; a_\text{max}^\text{GNN},\; a_{s,\text{max}}^\text{GNN}]$$

| パラメータ | 記号 | 値 | 単位 | 設定根拠 |
|---|---|---|---|---|
| 並進加速度スケール | $a_\text{max}^\text{GNN}$ | 1.0 | m/s² | 物理上限 $u_\text{max}/m = 3.0$ m/s² の 1/3 程度（学習安定化のため控えめに設定） |
| スケール加速度スケール | $a_{s,\text{max}}^\text{GNN}$ | 0.5 | s$^{-2}$ | 同上 |

$\odot$ は要素積．tanh により出力は $[-1,1]^3$ に収まり，スケール後は $\pi_\text{scaled} \in [-1.0, 1.0]^2 \text{ m/s}^2 \times [-0.5, 0.5] \text{ s}^{-2}$．

### 3.3 名目制御器（PD + スプリング復元力）

GNNによる学習済み加速度オフセットは，名目制御器の出力に直接加算される．名目制御器は以下の2段階階層構造を持つ．

#### Level 1：目標速度の生成（名目のみ，GNN介入なし）
...
#### Level 2：PD追従による名目加速度生成 + GNN加速度オフセット

名目加速度を計算し，GNNの出力を**加速度空間で直接加算**する：

$$a_{cx} = K_v \cdot (v_{\text{ref},x} - v_{\text{current},x}) + \pi_\text{scaled}[0]$$

$$a_{cy} = K_v \cdot (v_{\text{ref},y} - v_{\text{current},y}) + \pi_\text{scaled}[1]$$

$K_v = 2.0$ s$^{-1}$，出力単位は m/s²．

#### スケール制御（スプリング復元力 + GNN加速度オフセット）

スケールは $s_\text{max}$ 方向へのスプリング制御に GNN オフセットを加算する：

$$a_s = K_s \cdot (\dot{s}_\text{ref} - \dot{s}) + \pi_\text{scaled}[2]$$

$$\dot{s}_\text{ref} = K_{s,\text{pos}} \cdot (s_\text{max} - s)$$

| パラメータ | 記号 | 値 | 単位 |
|---|---|---|---|
| スケール速度ゲイン | $K_{s,\text{pos}}$ | 1.0 | s$^{-1}$ |
| スケールPDゲイン | $K_s$ | 2.0 | s$^{-1}$ |

#### 最終入力

$$u_\text{AT} = [a_{cx},\; a_{cy},\; a_s]^\top \in \mathbb{R}^3$$

GNNは名目加速度への**残差補正（加速度オフセット）**を学習する．速度経由の間接的な変換を挟まないため，制御則の構造がシンプルになる．
...
目標に近づいた量（距離減少）の期待値を最大化する．$p_{t+1}^\text{pred}$ は GNN の加速度オフセット $\pi_\text{scaled}$ から予測した1ステップ先位置であり，GNNへの勾配が流れる（係数 $c_\text{prog} = 1.0$）：

$$p_{t+1}^\text{pred} = p_t + v_t \Delta t + \frac{1}{2m}(a_\text{nom} + \pi_\text{scaled}[:2])\,\Delta t^2$$
...
[Phase 1: データ収集 (torch.no_grad)]
  環境リセット → 各タイムステップ:
    ...
    2. GNN π_φ(G) → tanh → π_scaled (加速度オフセット)
    3. Level 1: v_ref = K_pos * (goal - pos)  (名目のみ)
    4. Level 2: u_nom = PD(v_ref) + spring(s)
    5. u_AT = u_nom + π_scaled  (加速度空間で直接加算)
